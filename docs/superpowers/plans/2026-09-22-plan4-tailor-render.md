...
| 2 | `_give_up_resume` left `pdf_path` set, so a re-scored row could attach the **previous** run's PDF under a new score — and a set `pdf_path` also suppressed the "couldn't generate resume" notice. | Task 6 clears `pdf_path`/`page_overflow`; Task 8's runbook clears every downstream artifact when re-scoring. |
